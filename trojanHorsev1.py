...
def teacher_server(stop_event):
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        try:
            ...
            safe_print("Hacker messaging disabled.")
            return
        s.listen(1)
        safe_print(f"(Hacker message server listening on {HOST}:{PORT})")
                    ...
                    try:
                        ...
                        while b"\n" in buffer:
                            ...
                            if line:
                                ...
                                if line.strip() == "quit":
                                    display_teacher_message("(hacker requested end of game)")
                                    stop_event.set()
                                    break
                    except socket.timeout:
                        continue
                    except Exception as e:
                        safe_print(f"(hacker connection error: {e})")
                        break
                safe_print("(hacker disconnected)")
...
def number_guessing_game():
    ...
    while True:
        try:
            # We use a consistent prompt so the hacker  message display can reprint it.
